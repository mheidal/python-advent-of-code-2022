...
class Directory:
    ...
    def get_size(self) -> int:
        return sum(file.size for file in self.files) + sum(sub_dir.get_size() for sub_dir in self.sub_directories)
...
def cd(current_directory: Directory, target_directory: str, root_directory: Directory):
    ...
    return next(directory for directory in current_directory.sub_directories if directory.name == target_directory)
...
def get_all_directories(root: Directory) -> List[int]:
    ...
    def add_subdirs(directory: Directory):
        ...
        dirs.extend(subdir.get_size() for subdir in directory.sub_directories if len(directory.sub_directories) == 0)
