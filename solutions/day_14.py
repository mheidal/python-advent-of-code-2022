...
def get_grid(include_buffers: bool) -> Tuple[List[List[bool]], int, int]:
    with open(f"../inputs/day_14.txt", "r") as input_file:
        ...
        all_coords = [
            ...
            for coord in re.findall(r"\d+,\d+", input_text)
        ]
        ...
        for line in input_text.splitlines():
            ...
            for i in range(len(corner_points) - 1):
                ...
                if x1 == x2:
                    ...
                    grid[y1][min(x1, x2) : max(x1, x2) + 1] = [True] * (
                        abs(x2 - x1) + 1
                    )
...
def insert_sand_grain_worked(grid: List[List[bool]], x_offset, y_offset):
    ...
    while has_next:
        ...
        if not grid[pos_y + 1][pos_x]:
            ...
            if (pos_x == 0 and pos_y < len(grid) - 1 and grid[pos_y + 1][pos_x]) or (
                pos_x == len(grid[0]) - 1
                and pos_y < len(grid) - 1
                and grid[pos_y + 1][pos_x]
                and grid[pos_y + 1][pos_x - 1]
            ):
                return False
            else:
                has_next = False
